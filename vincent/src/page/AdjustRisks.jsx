...
import { Table, Card, Button, Form, Space, Select, message, Radio } from 'antd';
import { DownloadOutlined } from '@ant-design/icons';
import { updateResponse } from "@/api/adjustrisks";

export default function AdjustRisks() {
  ...
  const handleRadioChange = (e, index) => {
    ...
    updatedData[index].adjustment = e.target.value;
    data.responseData[index].adjustment = e.target.value;
  ...
  return (
    <>
        ...
        >
          ...
          <Column
            ...
            render={(_, record, index) => (
              <Radio.Group value={record.adjustment} onChange={(e) => handleRadioChange(e, index)} >
      ...
      <Space style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '20px'}}>
        <Button type="primary" style={{ display: 'flex', alignItems: 'center', padding: '20px 30px', }} onClick={handleSave}>
          Save
        </Button>
      </Space>
